...
class _ScreenGrafoSocialState extends State<ScreenGrafoSocial> {
  ...
  Set<String> _siguiendo = {}; // Usuarios que se están siguiendo
  bool _cargando = true;
  final String usuarioActual = userUsuario; // <- cambia por el usuario logueado
  ...
  Future<void> _seguirUsuario(String otro) async {
    try {
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text("Error al seguir a $otro: $e")));
    }
  }
  ...
  Future<void> _dejarDeSeguir(String otro) async {
    try {
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text("Error al dejar de seguir a $otro: $e")));
    }
  }
          ...
          : _usuarios.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    if (nombreUsuario == usuarioActual) return const SizedBox.shrink();

                    final bool siguiendo = _siguiendo.contains(nombreUsuario);

                    return Card(
                      margin: const EdgeInsets.symmetric(vertical: 6),
                      color: const Color(0xFFF8F5FF), // mismo tono violeta claro
                      shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(12),
                      ),
                      child: ListTile(
                        title: Text(nombreUsuario, style: const TextStyle(fontWeight: FontWeight.w600)),
                        subtitle: Text(nombreCompleto, style: const TextStyle(fontSize: 14)),
                        trailing: ElevatedButton(
                          ...
                          style: ElevatedButton.styleFrom(
                            backgroundColor: siguiendo ? Colors.red.shade100 : Colors.grey.shade300,
